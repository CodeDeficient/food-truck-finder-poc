...
test.describe('Pipeline Health Monitoring', () => {
    test('System Health Dashboard', async ({ request }) => {
        ...
        try {
            ...
            console.info(`   With Menus: ${stats.trucksWithMenus} (${((stats.trucksWithMenus / stats.totalTrucks) * 100).toFixed(1)}%)`);
            console.info(`   Without Menus: ${stats.trucksWithoutMenus} (${((stats.trucksWithoutMenus / stats.totalTrucks) * 100).toFixed(1)}%)`);
            console.info(`   Recent (24h): ${stats.recentTrucks}`);
            console.info(`   Stale (3+ days): ${stats.staleTrucks} (${((stats.staleTrucks / stats.totalTrucks) * 100).toFixed(1)}%)`);
            // Health assertions
        ...
        try {
            ...
            const qualityPercentage = (qualityReport.validTrucks / (qualityReport.validTrucks + qualityReport.invalidTrucks)) *
                100;
        ...
        try {
            ...
            const { data: recentActivity, error } = await supabaseAdmin
                ...
                .or(`created_at.gte.${twentyFourHoursAgo.toISOString()},updated_at.gte.${twentyFourHoursAgo.toISOString()}`)
    ...
    test('Performance Baseline Check', async ({ request }) => {
        ...
        console.log(`⏱️  Pipeline Processing Time: ${processingTime}ms (${(processingTime / 1000).toFixed(2)}s)`);
...
test.describe('Pipeline Alerts and Thresholds', () => {
    test('Critical Thresholds Check', async () => {
        ...
        if (stats.trucksWithoutMenus / stats.totalTrucks > 0.5) {
            alerts.push(`WARNING: ${((stats.trucksWithoutMenus / stats.totalTrucks) * 100).toFixed(1)}% of trucks missing menus`);
        }
        if (stats.staleTrucks / stats.totalTrucks > 0.7) {
            alerts.push(`WARNING: ${((stats.staleTrucks / stats.totalTrucks) * 100).toFixed(1)}% of trucks are stale (3+ days)`);
        }
