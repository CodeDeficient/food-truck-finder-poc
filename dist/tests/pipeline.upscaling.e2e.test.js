...
test.describe('Pipeline Upscaling E2E Tests', () => {
    ...
    test('Single URL Processing - Complete Pipeline', async ({ request }) => {
        ...
        // Poll for completion
        const truck = await pollForTruckByUrl(testUrl, POLLING_CONFIG.maxAttempts, POLLING_CONFIG.delay);
    ...
    test('Pipeline Performance - Processing Time', async ({ request }) => {
        ...
        console.log(`Pipeline processing time: ${processingTime}ms (${(processingTime / 1000).toFixed(2)}s)`);
    ...
    test('Stale Data Detection and Refresh', async ({ request }) => {
        ...
        if (staleTrucks && staleTrucks.length > 0) {
            ...
            const { data: refreshedTruck } = await supabaseAdmin
                ...
                .single();
            if (refreshedTruck) {
                console.log(`Stale truck check - Original: ${originalUpdatedAt}, Current: ${refreshedTruck.updated_at}`);
            }
        }
        else {
...
// Helper functions
async function cleanupTruckByUrl(url) {
...
async function pollForTruckByUrl(url, maxAttempts, delay) {
