...
function MetricsCards({ metrics }) {
    const successRate = metrics.totalRuns > 0 ? Math.round((metrics.successfulRuns / metrics.totalRuns) * 100) : 0;
    return (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <MetricCard title="Total Runs" value={metrics.totalRuns} subtitle={`${metrics.successfulRuns} successful`} icon={<RefreshCw className="size-4 text-muted-foreground"/>}/>
      <MetricCard title="Success Rate" value={`${successRate}%`} subtitle={`${metrics.failedRuns} failed runs`} icon={<CheckCircle className="size-4 text-muted-foreground"/>}/>
      <MetricCard title="Trucks Processed" value={metrics.totalTrucksProcessed} subtitle="Total trucks processed" icon={<Clock className="size-4 text-muted-foreground"/>}/>
      <MetricCard title="New Today" value={metrics.newTrucksToday} subtitle="New trucks found today" icon={<Play className="size-4 text-muted-foreground"/>}/>
    </div>);
}
...
*     name: 'Job1',
*     schedule: '*/
function CronJobRow({ job, getStatusIcon, getStatusBadge, }) {
    return (<TableRow>
      ...
      <TableCell>
        {job.lastResult ? (<div className="text-sm">
            <div className={job.lastResult.success ? 'text-green-600' : 'text-red-600'}>
              {job.lastResult.success ? 'Success' : 'Failed'}
            </div>
            {job.lastResult.trucksProcessed !== undefined && (<div className="text-xs text-muted-foreground">
 ...
 */
function CronJobsTable({ cronJobs, getStatusIcon, getStatusBadge, }) {
    return (<Card>
      <CardHeader>
      ...
      <CardContent>
        <Table className="w-full">
          ...
          <TableBody>
            {cronJobs.map((job) => (<CronJobRow key={job.id} job={job} getStatusIcon={getStatusIcon} getStatusBadge={getStatusBadge}/>))}
            {cronJobs.length === 0 && (<TableRow>
...
*/
function DashboardHeader({ onRefresh, onTriggerScrape, }) {
...
async function fetchCronJobs() {
    ...
    return Array.isArray(cronData.jobs)
        ? cronData.jobs
        : [];
}
...
export default function AutoScrapingDashboard() {
    ...
    return (<div className="space-y-6">
      ...
      <CronJobsTable cronJobs={cronJobs} getStatusIcon={getStatusIcon} getStatusBadge={getStatusBadge}/>
    </div>);
}
