...
export default function FoodTruckFinder() {
    const { loading, searchTerm, setSearchTerm, userLocation, loadNearbyTrucks, filteredTrucks, selectedTruckId, setSelectedTruckId, } = useFoodTruckFinder();
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-black">
      <AppHeader mounted={mounted} resolvedTheme={resolvedTheme} setTheme={setTheme} searchTerm={searchTerm} setSearchTerm={setSearchTerm} userLocation={userLocation} loadNearbyTrucks={() => {
            void loadNearbyTrucks();
        }}/>

      <MainContent filteredTrucks={filteredTrucks} userLocation={userLocation} selectedTruckId={selectedTruckId} setSelectedTruckId={setSelectedTruckId} isOpen={isTruckOpen} theme={resolvedTheme}/>
    </div>);
}
