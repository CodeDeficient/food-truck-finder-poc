...
const FoodTruckCard = ({ truck }) => {
    return (<div className="border rounded-lg p-4">
      ...
      {truck.events && truck.events.length > 0 && (<div className="mt-4">
          <h4 className="font-semibold mb-1">Upcoming Events</h4>
          <ul>
            {truck.events.slice(0, 3).map((event) => (<li key={event.id} className="mb-1">
                <span className="font-medium">{event.title}</span> &mdash; {event.event_date}
                {event.location_address !== undefined && event.location_address !== '' && (<span className="ml-2 text-gray-500">@ {event.location_address}</span>)}
              </li>))}
          </ul>
        </div>)}
    </div>);
