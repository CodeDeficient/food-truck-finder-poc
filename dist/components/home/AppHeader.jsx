 ...
 */
export function AppHeader({ mounted, resolvedTheme, setTheme, searchTerm, setSearchTerm, userLocation, loadNearbyTrucks, }) {
    return (<header className="bg-white dark:bg-slate-800 shadow-md p-4 flex items-center justify-between">
      <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Food Truck Finder</h1>
      <div className="flex items-center gap-4">
        <Input type="text" placeholder="Search for trucks..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-64"/>
        {userLocation && (<Button onClick={loadNearbyTrucks} variant="outline">
            Nearby Trucks
          </Button>)}
        {mounted && (<Button variant="outline" size="icon" onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}>
