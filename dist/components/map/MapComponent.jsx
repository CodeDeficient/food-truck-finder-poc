...
const getFoodTruckIcon = (isOpen, theme) => {
...
const MapComponent = ({ trucks, userLocation, defaultCenter, defaultZoom = 10, onSelectTruck, selectedTruckLocation, theme, }) => {
    const [isMounted, setIsMounted] = useState(false);
    const [trucksWithCoords, setTrucksWithCoords] = useState([]);
    useEffect(() => {
        setIsMounted(true);
    }, []);
    ...
    const initialMapCenter = userLocation && typeof userLocation.lat === 'number' && typeof userLocation.lng === 'number'
    ...
    if (!isMounted) {
        return (<div className={`map-loading rounded-lg shadow-md ${isDark ? 'dark' : ''}`} style={{
                height: '400px',
                width: '100%',
                display: 'flex',
                alignItems: 'center',
    ...
    return (<MapContainer center={initialMapCenter} zoom={defaultZoom} scrollWheelZoom={true} zoomControl={true} doubleClickZoom={true} dragging={true} style={{ height: '400px', width: '100%' }} className="rounded-lg shadow-md" 
    // Enhanced performance settings
    preferCanvas={false}>
      <TileLayer attribution={tileLayerProps.attribution} url={tileLayerProps.url} maxZoom={19} minZoom={1} tileSize={256} zoomOffset={0} 
    // Performance enhancements
    keepBuffer={2} updateWhenIdle={true} updateWhenZooming={false} 
    // Better visual quality
    className={tileLayerProps.className}/>
      <MapViewUpdater center={selectedTruckLocation} zoom={selectedTruckLocation ? 13 : undefined}/>
      {validTrucks.map((truck) => {
            const isOpen = isTruckOpen(truck);
            return (<Marker key={truck.id} position={[truck.current_location.lat, truck.current_location.lng]} icon={getFoodTruckIcon(isOpen, theme || 'light')} eventHandlers={{
                    click: () => {
                        if (onSelectTruck) {
