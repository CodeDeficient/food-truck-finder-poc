...
*/
const TruckMarkers = ({ trucks, onSelectTruck }) => {
    const validTrucks = trucks.filter((truck) => truck.current_location != undefined &&
        typeof truck.current_location.lat === 'number' &&
        typeof truck.current_location.lng === 'number');
    return (<>
      {validTrucks.map((truck) => (<Marker key={truck.id} position={[truck.current_location.lat, truck.current_location.lng]} icon={foodTruckIcon} eventHandlers={{
                click: () => {
                    if (onSelectTruck) {
