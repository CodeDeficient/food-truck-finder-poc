 ...
 */
function CuisineTypeSection({ cuisineType }) {
    if (cuisineType == undefined || cuisineType.length === 0)
        return;
    return (<div>
      <label className="text-sm font-medium text-gray-500">Cuisine Type</label>
      <div className="flex flex-wrap gap-1 mt-1">
        {cuisineType.map((cuisine, index) => (<Badge key={index} variant="secondary">
            {cuisine}
          </Badge>))}
      </div>
    </div>);
 ...
 */
export function BasicInfoCard({ truck }) {
    return (<Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {truck.description !== undefined && truck.description !== '' && (<div>
