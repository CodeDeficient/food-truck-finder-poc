 ...
 */
export function OperationSelector({ selectedOperations, onToggleOperation, }) {
    return (<Card>
      <CardHeader>
      ...
      <CardContent>
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {Object.entries(operationConfig).map(([key, config]) => (<div key={key} className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${selectedOperations.includes(key)
                ? 'border-blue-500 bg-blue-50 dark:bg-blue-950'
                : 'border-gray-200 hover:border-gray-300'}`} onClick={() => onToggleOperation(key)}>
              <div className={config.color}>{config.icon}</div>
              <div className="flex-1">
                <div className="font-medium text-sm">{config.name}</div>
                <div className="text-xs text-muted-foreground">{config.description}</div>
              </div>
              {selectedOperations.includes(key) && (<CheckCircle className="size-4 text-blue-600"/>)}
            </div>))}
        </div>
      </CardContent>
    </Card>);
