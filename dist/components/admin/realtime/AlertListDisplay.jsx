 ...
 */
export function AlertListDisplay({ alerts, showDetails, onToggleDetails, onAcknowledgeAlert, }) {
    const visibleAlerts = showDetails ? alerts : alerts.slice(0, 3);
    return (<>
      <div className="space-y-2">
        {visibleAlerts.map((alert) => (<SystemAlertItem key={alert.id} alert={alert} onAcknowledgeAlert={onAcknowledgeAlert}/>))}
      </div>
      <AlertToggleButton alertsLength={alerts.length} showDetails={showDetails} onToggleDetails={onToggleDetails}/>
    </>);
}
