 ...
 */
function RealtimeStatusDisplayContent({ connectionError, systemMetrics, scrapingJobs, alerts, showDetails, onToggleDetails, onAcknowledgeAlert, recentEventsCount, onClearEvents, }) {
    return (<CardContent>
      {connectionError !== undefined && (<div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
          <p className="text-sm text-red-600">{connectionError}</p>
        </div>)}
      <SystemMetricsGrid metrics={systemMetrics}/>
      <ScrapingJobsStatus scrapingJobs={scrapingJobs}/>
      <SystemAlerts alerts={alerts} showDetails={showDetails} onToggleDetails={onToggleDetails} onAcknowledgeAlert={onAcknowledgeAlert}/>
 ...
 */
export function RealtimeStatusDisplay({ isConnected, isConnecting, connectionError, lastEventTime, connect, disconnect, systemMetrics, scrapingJobs, alerts, showDetails, onToggleDetails, onAcknowledgeAlert, recentEventsCount, onClearEvents, }) {
    return (<div className="space-y-4">
      <Card className={`border-l-4 ${isConnected ? 'border-l-green-500' : 'border-l-red-500'}`}>
        <ConnectionStatusHeader isConnected={isConnected} isConnecting={isConnecting} lastEventTime={lastEventTime ? new Date(lastEventTime) : undefined} connect={connect} disconnect={disconnect}/>
        <RealtimeStatusDisplayContent connectionError={connectionError} systemMetrics={systemMetrics} scrapingJobs={scrapingJobs} alerts={alerts} showDetails={showDetails} onToggleDetails={onToggleDetails} onAcknowledgeAlert={onAcknowledgeAlert} recentEventsCount={recentEventsCount} onClearEvents={onClearEvents}/>
      </Card>
    </div>);
}
