...
*/
export function MenuSection({ popularItems }) {
    return (<div>
      <h4 className="font-medium mb-2 text-sm dark:text-gray-100">Popular Items</h4>
      <div className="space-y-1">
        {popularItems.map((item, idx) => (<div key={idx} className="flex justify-between text-sm dark:text-gray-300">
            <span className="truncate dark:text-gray-200">{item.name}</span>
            {/* Handle price as string or number */}
            {item.price !== undefined && (<span className="text-green-600 dark:text-green-400 ml-2">
