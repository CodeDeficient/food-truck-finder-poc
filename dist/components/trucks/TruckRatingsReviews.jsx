...
function StarRating({ rating }) {
    return (<div className="flex">
      {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={`size-6 ${star <= Math.round(rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}/>))}
    </div>);
}
/**
 ...
 */
export function TruckRatingsReviews({ truck }) {
    ...
    return (<Card className="dark:bg-slate-800 dark:border-slate-700">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-4">
          ...
          {truck.review_count !== undefined && (<div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
