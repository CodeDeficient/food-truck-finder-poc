...
export async function handleScrapeOperation(url, options) {
    const scrapeResult = await firecrawl.scrapeUrl(url, Object.assign({ formats: ['markdown', 'html'], onlyMainContent: true }, options));
...
export async function handleCrawlOperation(url, options) {
    ...
    if (!crawlJob.success || crawlJob.jobId == undefined) {
        return NextResponse.json({
            success: false,
            error: (_a = crawlJob.error) !== null && _a !== void 0 ? _a : 'Failed to start crawl job',
        }, { status: 500 });
    }
