    ...
    */
    ...
    static async normalizePhoneNumbers(trucks, dryRun, operation) {
        const promises = trucks.map((truck) => {
            var _b;
            if (((_b = truck.contact_info) === null || _b === void 0 ? void 0 : _b.phone) !== undefined) {
        ...
        if (dryRun) {
            ...
            try {
                await FoodTruckService.updateTruck(truck.id, {
                    contact_info: Object.assign(Object.assign({}, truck.contact_info), { phone: normalizedPhone }),
                });
                operation.successCount += 1;
            }
            catch (error) {
    ...
    static async fixCoordinates(trucks, dryRun, operation) {
        ...
        const promises = trucks.map((truck) => this.processSingleTruckCoordinates(truck, Object.assign(Object.assign({}, context), { operation })));
        await Promise.all(promises);
        return operation;
    }
    /**
    ...
    static async applyCoordinateFixUpdate(truck, updates, dryRun, operation) {
        ...
        if (dryRun) {
            ...
            try {
                await FoodTruckService.updateTruck(truck.id, {
                    current_location: Object.assign(Object.assign({}, truck.current_location), updates),
                });
                operation.successCount += 1;
            }
            catch (error) {
    ...
    */
    static async processSingleTruckForQualityScore(truck, dryRun, operation) {
        ...
        if (qualityAssessment) {
            ...
            const currentScore = (_b = truck.data_quality_score) !== null && _b !== void 0 ? _b : 0;
    ...
    static async processSingleTruckForDuplicates(truck, dryRun, context) {
        ...
        if (duplicateCheck.isDuplicate &&
            ((_b = duplicateCheck.bestMatch) === null || _b === void 0 ? void 0 : _b.confidence) === 'high' &&
    ...
    static getOperationDescription(type) {
        ...
        return (_b = descriptions[type]) !== null && _b !== void 0 ? _b : 'Unknown operation';
    }
    /**
    ...
    static calculateSummary(operations) {
        ...
        return {
            ...
            duplicatesRemoved: (_c = (_b = operations.find((op) => op.type === 'merge_duplicates')) === null || _b === void 0 ? void 0 : _b.successCount) !== null && _c !== void 0 ? _c : 0,
            qualityScoreImprovement: (_e = (_d = operations.find((op) => op.type === 'update_quality_scores')) === null || _d === void 0 ? void 0 : _d.successCount) !== null && _e !== void 0 ? _e : 0,
            placeholdersRemoved: (_g = (_f = operations.find((op) => op.type === 'remove_placeholders')) === null || _f === void 0 ? void 0 : _f.successCount) !== null && _g !== void 0 ? _g : 0,
        };
    }
}
_a = BatchCleanupService;
BatchCleanupService.runRemovePlaceholders = async (trucks, dryRun, operation) => {
    return await _a.removePlaceholders(trucks, dryRun, operation);
};
BatchCleanupService.runNormalizePhoneNumbers = async (trucks, dryRun, operation) => {
    return await _a.normalizePhoneNumbers(trucks, dryRun, operation);
};
BatchCleanupService.runFixCoordinates = async (trucks, dryRun, operation) => {
    return await _a.fixCoordinates(trucks, dryRun, operation);
};
BatchCleanupService.runUpdateQualityScores = async (trucks, dryRun, operation) => {
    return await _a.updateQualityScores(trucks, dryRun, operation);
};
BatchCleanupService.runMergeDuplicates = async (trucks, dryRun, operation) => {
    return await _a.mergeDuplicates(trucks, dryRun, operation);
};
