...
export function DataStatusIndicator({ status, }) {
    ...
    if (status.status === 'cached') {
        return (React.createElement("div", { className: "bg-blue-100 border border-blue-300 text-blue-800 px-4 py-2 rounded" },
            "Showing cached data from ",
            status.lastUpdate,
            ". Live data temporarily unavailable."));
    }
    if (status.status === 'stale') {
        return (React.createElement("div", { className: "bg-yellow-100 border border-yellow-300 text-yellow-800 px-4 py-2 rounded" },
            "Showing older data from ",
            status.lastUpdate,
            ". We're working to restore live updates."));
    }
    if (status.status === 'unavailable') {
        return (React.createElement("div", { className: "bg-red-100 border border-red-300 text-red-800 px-4 py-2 rounded" }, "Service temporarily unavailable. Please check back in a few minutes."));
    }
