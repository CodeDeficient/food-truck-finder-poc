...
async function handleScraping(targetUrl, jobId) {
    ...
    if (!scrapeResult.success || !((_a = scrapeResult.data) === null || _a === void 0 ? void 0 : _a.markdown)) {
        const error = (_b = scrapeResult.error) !== null && _b !== void 0 ? _b : 'Scraping failed or markdown content not found';
...
async function handleGeminiExtraction(markdown, sourceUrl, jobId) {
    ...
    if (!geminiResult.success || !geminiResult.data) {
        const error = (_a = geminiResult.error) !== null && _a !== void 0 ? _a : 'Gemini data extraction failed';
        await handleJobFailure(jobId, error);
        throw new Error(error);
    }
...
export async function processScrapingJob(jobId) {
    ...
    try {
        ...
        const extractedData = await handleGeminiExtraction(scrapeData.markdown, (_a = scrapeData.source_url) !== null && _a !== void 0 ? _a : job.target_url, jobId);
        await ScrapingJobService.updateJobStatus(jobId, 'completed', {
            data_collected: extractedData,
            completed_at: new Date().toISOString(),
        });
        await createOrUpdateFoodTruck(jobId, extractedData, (_b = scrapeData.source_url) !== null && _b !== void 0 ? _b : job.target_url);
...
export async function createOrUpdateFoodTruck(jobId, extractedTruckData, sourceUrl) {
    try {
        ...
        console.error(`Job ${jobId}: Error in createOrUpdateFoodTruck from ${sourceUrl !== null && sourceUrl !== void 0 ? sourceUrl : 'Unknown Source'}:`, error);
