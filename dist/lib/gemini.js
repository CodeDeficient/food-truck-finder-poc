...
export class GeminiService {
    constructor() {
        ...
        this.makeGeminiRequest = async (prompt, parser) => {
            return this.apiClient.makeRequestWithParsing(prompt, parser);
        };
        this.processMenuData = async (rawMenuText) => {
        ...
        this.extractLocationFromText = async (textInput) => {
        ...
        this.standardizeOperatingHours = async (hoursText) => {
            const usageCheck = await this.checkUsageLimits();
            if (!usageCheck.canMakeRequest) {
                console.error('Gemini API usage limit error: Daily API limits exceeded');
                return {
        ...
        this.analyzeSentiment = async (reviewText) => {
            const usageCheck = await this.checkUsageLimits();
            if (!usageCheck.canMakeRequest) {
                console.error('Gemini API usage limit error: Daily API limits exceeded');
                return {
        ...
        this.enhanceFoodTruckData = async (rawData) => {
            const usageCheck = await this.checkUsageLimits();
            if (!usageCheck.canMakeRequest) {
                console.error('Gemini API usage limit error: Daily API limits exceeded');
                return {
        ...
        this.getUsageStats = async () => {
            const usage = await APIUsageService.getTodayUsage('gemini');
            return usage !== null && usage !== void 0 ? usage : undefined;
        };
        this.extractFoodTruckDetailsFromMarkdown = async (markdownContent, sourceUrl) => {
            const usageCheck = await this.checkUsageLimits();
            if (!usageCheck.canMakeRequest) {
            ...
            return Object.assign(Object.assign({}, response), { promptSent: prompt });
        };
