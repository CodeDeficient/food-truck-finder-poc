      ...
      case 'status': {
        return NextResponse.json({
          ...
          summary: {
            ...
            averageSuccessRate:
              scraperConfigs.reduce((acc, s) => acc + s.successRate, 0) / scraperConfigs.length,
          },
        });
      }
...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as { scraperId: string /* priority?: string */ }; // Priority removed as it's unused
    const { scraperId /*, priority = "normal" */ } = body; // Priority removed
...
export async function PUT(request: NextRequest) {
  try {
    const body = (await request.json()) as { scraperId: string; config: Partial<ScraperConfig> };
