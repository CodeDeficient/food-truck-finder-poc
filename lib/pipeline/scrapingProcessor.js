...
export async function createOrUpdateFoodTruck(jobId, extractedTruckData, sourceUrl) {
    try {
        if (extractedTruckData.name === null) {
            console.warn(`Job ${jobId}: Discarding truck from ${sourceUrl} due to null name.`);
            await ScrapingJobService.updateJobStatus(jobId, 'completed', {
                data_collected: extractedTruckData,
                completed_at: new Date().toISOString(),
                notes: 'Discarded due to null name',
            });
            return;
        }
