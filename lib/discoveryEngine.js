    ...
    */
    ...
    async storeDiscoveredUrl(url, discoveryMethod = 'manual', metadata = {}) {
        try {
            ...
            // Pre-filter URLs that are obviously not food truck related
            if (this.isObviouslyNonFoodTruckUrl(url)) {
                console.info(`⏭️  Skipping obviously non-food truck URL: ${url}`);
                return { isNew: false };
            }
            ...
            if (existing && existing.length > 0) {
                // If URL exists but was blacklisted, don't re-add it
                if (existing[0].status === 'blacklisted') {
            ...
            // Store new URL with quality tracking fields
            const insertData = {
                url,
                status: 'new',
     ...
     */
    isObviouslyNonFoodTruckUrl(url) {
    ...
    /**
     * Update URL quality score based on processing results
     */
    async updateUrlQualityScore(url, result) {
        try {
            if (!supabaseAdmin) {
                return;
            }
