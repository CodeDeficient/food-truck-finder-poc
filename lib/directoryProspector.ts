...
export async function prospectForCandidateDirectories(): Promise<string[]> {
  ...
  for (const seedUrl of SC_DIRECTORY_PROSPECTOR_SEED_URLS) {
    ...
    try {
      const firecrawlOptions = { pageOptions: { format: 'html' } }; // Firecrawl now primarily uses `content` for HTML if format is 'html'
      const scrapeResult = await firecrawl.scrapeUrl(seedUrl, firecrawlOptions);
      // console.info(`DirectoryProspector: Raw scrapeResult for ${seedUrl}:`, JSON.stringify(scrapeResult, null, 2)); // Optional: for deep debugging

      const htmlContentForLinks = scrapeResult.data?.html || scrapeResult.data?.content;
      if (scrapeResult.success && htmlContentForLinks) {
        const baseOrigin = new URL(seedUrl).origin;
        const linksFromSeed = extractAbsoluteLinks(htmlContentForLinks, baseOrigin);
      ...
      console.error(`DirectoryProspector: Exception while processing seed URL ${seedUrl}:`, error.message || String(error));
    }
  }
  ...
  for (const query of SC_DIRECTORY_PROSPECTOR_SEARCH_QUERIES) {
    const googleSearchUrl = `https://www.google.com/search?q=${encodeURIComponent(query)}&hl=en&gl=us`;
    ...
    try {
      const serpResult = await firecrawl.scrapeUrl(googleSearchUrl, {
        ...
        // scraperOptions can be added if Firecrawl supports specific ones for SERPs
      });
      // console.info(`DirectoryProspector: Raw SERP scrapeResult for query "${query}":`, JSON.stringify(serpResult, null, 2)); // Optional: for deep debugging

      const htmlContentForSerpLinks = serpResult.data?.html || serpResult.data?.content;
      if (serpResult.success && htmlContentForSerpLinks) {
        const linksFromSerp = extractAbsoluteLinks(htmlContentForSerpLinks, 'https://www.google.com');
      ...
      console.error(`DirectoryProspector: Exception while processing SERP for query "${query}":`, error.message || String(error));
    }
  }
