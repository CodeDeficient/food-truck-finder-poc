...
class UnusedCodeCleaner {
   ...
   */
  async run(options = {}) {
    const { createBackup = true, verifyCompilation = true, autoRollback = true } = options;
    ...
    try {
      ...
      console.log(
        `Unused variables: ${initialSpecific.unusedVars} → ${finalSpecific.unusedVars} (${unusedVarsFixed} fixed)`,
      );
      console.log(
        `Unused imports: ${initialSpecific.unusedImports} → ${finalSpecific.unusedImports} (${unusedImportsFixed} fixed)`,
      );
