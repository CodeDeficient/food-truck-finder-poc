    ...
    for message in item.get("messages", []):
        if message.get("ruleId") is not None:
            if message.get("severity") == 2:  # ESLint severity 2 is an error
                errors_by_rule[message.get("ruleId")] = errors_by_rule.get(
                    message.get("ruleId"), 0) + 1
            elif message.get("severity") == 1:  # ESLint severity 1 is a warning
                warnings_by_rule[message.get("ruleId")] = warnings_by_rule.get(
                    message.get("ruleId"), 0) + 1

print(f"Total Errors: {total_errors}")
print(f"Total Warnings: {total_warnings}")
print(f"Fatal Errors: {total_fatal_errors}")
